        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="image-container bg-gray-800 shadow-xl rounded-lg mb-12 p-12 clear-both">
                    <img id="nome" src="{{asset('storage/images/schede/'.$user->id.'.png')}}" alt="Immagine">
                    @foreach($s as $k => $v)
                        <img id="{{$k}}" src="{{asset('storage/images/schede/'.$user->id.'.png')}}" style="display: {{$v}}" alt="{{$k}}">
                    @endforeach
                    <img id="tutta" src="{{asset('storage/images/schede/'.$user->id.'.png')}}" alt="Immagine">

                </div>
                @if($user->spells)
                    <div class="image-container bg-gray-800 shadow-xl rounded-lg mb-12 p-12 clear-both">
                        <img id="spell" style="display: {{$spell}}" src="{{asset('storage/images/schede/'.$user->id.'s.png')}}" alt="Spell">
                    </div>
                @endif
